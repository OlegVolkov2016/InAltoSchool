...
public class StudentsLessonDialogController {
	...
	private void getListStudents(ObservableList<User> listStudents) {
		...
		ResultSet result = UsersTable.getInstance().executeSelect("", usersTable+".user_id, CONCAT(user_firstname, ' ', user_lastname) ", "FROM "+usersTable
				+" JOIN "+coursesExecutionsTable+" ON "+usersTable+".user_id = "+coursesExecutionsTable+".executor_id", " WHERE ("+coursesExecutionsTable+".course_id = "+course.getCourse_id()
				+") AND ("+coursesExecutionsTable+".group_id IS NULL)", "", "", "");
		try {
			while (result.next()) {
				listStudents.add(new User(result.getInt(1),0,result.getString(2),"User",null,"","","","","","",0,"",0,""));
			}
