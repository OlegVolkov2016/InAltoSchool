...
public class ProjectsTasksSplitPaneController {
	...
	private void addButton() {
		Task task = tableTasks.getSelectionModel().getSelectedItem();
		Stage dialogStage = new Stage();
		StudentsProjectTaskDialog studentsDialog = new StudentsProjectTaskDialog(dialogStage);
		studentsDialog.getController().setProject(project);
		studentsDialog.getController().setTask(task);
		...
		try {
			if (result.next()) {
				...
				Execution execution = new Execution(result.getInt(1),result.getInt(2),result.getInt(3),0,startDate,endDate,
						result.getInt(6),result.getInt(7),task.getTask_name(),user.getUser_firstname());
				Stage dialogStage = new Stage();
				ProjectTaskStatusDialog statusDialog = new ProjectTaskStatusDialog(dialogStage);
	...
	private void newButton() {
		Task task = new Task(0,0,project.getProject_id(),"","","","",project.getProject_name(),project.getAuthor_name());
		Stage dialogStage = new Stage();
		ProjectTaskDialog projectTaskDialog = new ProjectTaskDialog(dialogStage);
		projectTaskDialog.getController().setTask(task);
        Scene scene = new Scene(projectTaskDialog);
        dialogStage.setScene(scene);
        dialogStage.initOwner(mainApp.getPrimaryStage());
        dialogStage.setMinWidth(((Pane) scene.getRoot()).getPrefWidth()+16);
        dialogStage.setMinHeight(((Pane) scene.getRoot()).getPrefHeight()+38);
        dialogStage.showAndWait();
        if (projectTaskDialog.getController().isOKButton()) {
        	updateTask(projectTaskDialog.getController().getTask());
        }
	}
	...
	public void changeLanguage () {
		...
		labelStudents.setText(Strings.LabelExecutors.getValue(Indexes.AppLanguage_index.getValue()));
		...
		projectsButton.setText(Strings.ProjectsButton.getValue(Indexes.AppLanguage_index.getValue()));
	}
	...
	public void changeLogin () {
		...
		if (project != null) {
			if (isAccessProject(getMainApp().getLogin(), project)) {
				if ((getMainApp().getLogin().getRole_name().equals("Student")) ||
						((getMainApp().getLogin().getUser_id() != project.getAuthor_id()) && 
				...
				projectsButton();
				((ProjectsSplitPane) getMainApp().getRootLayout().getMainSplitPane()).getController().getTableProjects().getSelectionModel().clearSelection();
			}
		}
	}
			...
			public void changed(ObservableValue<? extends Task> observable, Task oldValue, Task newValue) {
				...
				if (newValue != null) {
					...
					if ((mainApp.getLogin() != null) && (isAccessTask(mainApp.getLogin(),newValue))) {
						...
						projectsButton.setDisable(false);
						if ((!mainApp.getLogin().getRole_name().equals("Administrator")) && (mainApp.getLogin().getUser_id() != project.getAuthor_id())) {
							viewButton.setDisable(false);
							addButton.setDisable(true);
							removeButton.setDisable(true);
							statusButton.setDisable(true);
	...
	private void updateTask(Task task) {
		...
		if (task.getTask_id() == 0) {
			result = TasksTable.getInstance().executeSelect("", "task_id", "FROM "+tasksTable, 
					"WHERE (project_id = "+task.getProject_id()+") AND (task_name = \'"+task.getTask_name()+"\')", "", "", "");
			try {
				if (!result.next())
	...
	public void setProject(Project project) {
		...
		textAuthorName.setText(project.getAuthor_name());
